...
// .wrangler/tmp/bundle-8uGZYE/checked-fetch.js
...
// .wrangler/tmp/pages-vZxsLz/functionsWorker-0.2986485560156067.mjs
...
__name(dbBatch, "dbBatch");
__name2(dbBatch, "dbBatch");
...
function computeRiskOffForFill(takerNet, takerBasis, makerNet, makerBasis, takerSide, priceCents, qtyContracts) {
  const makerSide = takerSide === "bid" ? "ask" : "bid";
  const closeQtyTaker = takerSide === "bid" && takerNet < 0 ? Math.min(qtyContracts, Math.abs(takerNet)) : takerSide === "ask" && takerNet > 0 ? Math.min(qtyContracts, takerNet) : 0;
  const closeQtyMaker = makerSide === "bid" && makerNet < 0 ? Math.min(qtyContracts, Math.abs(makerNet)) : makerSide === "ask" && makerNet > 0 ? Math.min(qtyContracts, makerNet) : 0;
  const riskOffContracts = closeQtyTaker + closeQtyMaker;
  let riskOffPriceDiffCents = 0;
  if (closeQtyTaker > 0 && takerBasis > 0) {
    riskOffPriceDiffCents = takerNet < 0 ? (takerBasis - priceCents) * closeQtyTaker : (priceCents - takerBasis) * closeQtyTaker;
  }
  let riskOffPriceDiffMakerCents = 0;
  if (closeQtyMaker > 0 && makerBasis > 0) {
    riskOffPriceDiffMakerCents = makerNet < 0 ? (makerBasis - priceCents) * closeQtyMaker : (priceCents - makerBasis) * closeQtyMaker;
  }
  return {
    riskOffContracts,
...
__name(computeRiskOffForFill, "computeRiskOffForFill");
__name2(computeRiskOffForFill, "computeRiskOffForFill");
async function createTrade(db, _marketId, _takerOrderId, makerOrderIdParam, priceCents, qtyContracts, outcomeId, takerUserId, makerUserId, takerSide, riskOffContractsTaker = 0, riskOffContractsMaker = 0, riskOffPriceDiffTakerCents = 0, riskOffPriceDiffMakerCents = 0) {
  const token = crypto.randomUUID();
  const createTime = Math.floor(Date.now() / 1e3);
  const totalContracts = riskOffContractsTaker + riskOffContractsMaker;
  const makerOrderId = makerOrderIdParam != null && makerOrderIdParam !== "" && Number(makerOrderIdParam) > 0 ? typeof makerOrderIdParam === "string" ? parseInt(makerOrderIdParam, 10) : makerOrderIdParam : null;
...
function computePositionUpdate(currentNet, currentBasis, currentClosedProfit, side, priceCents, qtyContracts) {
...
__name(computePositionUpdate, "computePositionUpdate");
__name2(computePositionUpdate, "computePositionUpdate");
async function updatePositionsForFill(db, outcomeId, takerUserId, makerUserId, takerSide, priceCents, qtyContracts) {
  const now = Math.floor(Date.now() / 1e3);
  const takerDb = await dbFirst(
    db,
    "SELECT id, net_position, price_basis, closed_profit FROM positions WHERE outcome = ? AND user_id = ?",
    [outcomeId, takerUserId]
  );
  const makerDb = await dbFirst(
    db,
    "SELECT id, net_position, price_basis, closed_profit FROM positions WHERE outcome = ? AND user_id = ?",
...
__name(updatePositionsForFill, "updatePositionsForFill");
__name2(updatePositionsForFill, "updatePositionsForFill");
async function updatePosition(db, outcomeId, userId, side, priceCents, qtyContracts) {
  ...
  if (!positionDb) {
    await dbRun(
      ...
      `INSERT INTO positions (user_id, outcome, net_position, price_basis, closed_profit, settled_profit, is_settled, create_time)
       VALUES (?, ?, ?, ?, ?, 0, 0, ?)`,
      [userIdNum, outcomeId, newNetPosition, newPriceBasis, newClosedProfit, Math.floor(Date.now() / 1e3)]
    );
  } else {
    await dbRun(
      db,
...
async function addSystemClosedProfitOffset(db, outcomeId, closedProfitOffsetCents, netPositionDelta, fillPriceCents) {
  const now = Math.floor(Date.now() / 1e3);
  ...
  if (existing) {
    ...
    const basisClamped = initialNet !== 0 && initialBasis > 0 ? Math.max(PRICE_BASIS_MIN_CENTS, Math.min(PRICE_BASIS_MAX_CENTS, initialBasis)) : initialBasis;
    await dbRun(
      db,
      `INSERT INTO positions (user_id, outcome, net_position, price_basis, closed_profit, settled_profit, is_settled, create_time)
       VALUES (?, ?, ?, ?, ?, 0, 0, ?)`,
...
__name(addSystemClosedProfitOffset, "addSystemClosedProfitOffset");
__name2(addSystemClosedProfitOffset, "addSystemClosedProfitOffset");
async function replayOutcomePositions(db, outcomeId) {
...
__name(replayOutcomePositions, "replayOutcomePositions");
__name2(replayOutcomePositions, "replayOutcomePositions");
...
function slugForOutcomeId(s) {
  return (s || "").trim().toLowerCase().replace(/\s+/g, "-").replace(/\./g, "_").replace(/[^a-z0-9_-]/g, "").replace(/-+/g, "-").replace(/^\-|\-$/g, "") || "x";
}
__name(slugForOutcomeId, "slugForOutcomeId");
__name2(slugForOutcomeId, "slugForOutcomeId");
...
__name2(initials, "initials");
var AUCTION_PRICE_CENTS = 5e3;
var CONTRACTS_PER_TRADE = 1;
function isParsMarket(market) {
...
__name(isParsMarket, "isParsMarket");
__name2(isParsMarket, "isParsMarket");
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const round = body?.round != null ? Number(body.round) : void 0;
    ...
    for (let i = 0; i < bids.length; i++) {
      const b = bids[i];
      const uid = b?.user_id != null ? Number(b.user_id) : void 0;
      const guess = b?.guess != null ? Number(b.guess) : void 0;
    ...
    const userIds = new Set(bids.map((b) => Number(b.user_id)));
    for (const uid of userIds) {
      const user = await dbFirst(db, "SELECT id FROM users WHERE id = ?", [uid]);
      if (!user) {
    ...
    const N = bids.length;
    let outcomeId;
    let resolvedMarketId;
    let strikeStr;
    let participantName = "";
    if (isOutcome) {
      const outcomeRow = await dbFirst(
        db,
      ...
      const strikeNum = bids.reduce((s, b) => s + Number(b.guess), 0) / N;
      ...
      const participant = await dbFirst(
        db,
      ...
      if (isPars) {
        ...
        if (!market) {
          await dbRun(
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [newMarketId, marketShortName, marketSymbol, 12, 1, Math.floor(Date.now() / 1e3), "pars"]
          );
        ...
        let market = await dbFirst(db, "SELECT market_id FROM markets WHERE market_id = ?", [newMarketId]);
        if (!market) {
          market = await dbFirst(
            db,
        ...
        if (!market) {
          await dbRun(
            ...
            `INSERT INTO markets (market_id, short_name, symbol, max_winners, min_winners, created_date, market_type)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [newMarketId, marketShortName, marketSymbol, 12, 1, Math.floor(Date.now() / 1e3), "round_ou"]
          );
          market = { market_id: newMarketId };
        }
        resolvedMarketId = market.market_id;
    ...
    let outcome = await dbFirst(
      db,
      "SELECT outcome_id FROM outcomes WHERE outcome_id = ? AND market_id = ?",
      [outcomeId, resolvedMarketId]
    );
    if (!outcome) {
      const outcomeName = isPars ? `${participantName} Over ${strikeStr}` : `${participantName} Over ${strikeStr} - Round ${round}`;
      const ticker = isPars ? `${initials(participantName)}-OV-${strikeStr.replace(".", "_")}` : `${initials(participantName)}-OV-R${round}-${strikeStr.replace(".", "_")}`;
      await dbRun(
        db,
        `INSERT INTO outcomes (outcome_id, name, ticker, market_id, strike, created_date)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [outcomeId, outcomeName, ticker, resolvedMarketId, strikeStr, Math.floor(Date.now() / 1e3)]
      );
    }
    ...
    const shortUserIds = sortedBids.slice(0, half).map((b) => Number(b.user_id));
    const longUserIds = sortedBids.slice(half).map((b) => Number(b.user_id));
    const avgBid = bids.reduce((s, b) => s + Number(b.guess), 0) / N;
    const auctionPriceCents = isRoundOu ? AUCTION_PRICE_CENTS : Math.max(100, Math.min(9900, Math.round(avgBid * 100)));
    const createTime = Math.floor(Date.now() / 1e3);
    let tradesCreated = 0;
...
var onRequestPatch2 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const db = getDb(env);
  const existing = await dbFirst(db, "SELECT id FROM trades WHERE id = ?", [tradeId]);
  if (!existing) {
  ...
  if (riskOffContractsTaker !== void 0) {
  ...
  if (riskOffContractsMaker !== void 0) {
...
var onRequestDelete = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    if (makerOrderId != null && makerOrderId > 0) {
      const orderRow = await dbFirst(db, "SELECT id, contract_size, original_contract_size, status FROM orders WHERE id = ?", [makerOrderId]);
      if (orderRow) {
        ...
        const newStatus = newContractSize >= originalSize ? "open" : "partial";
...
var orderSchema = external_exports.object({
  outcome_id: external_exports.string().trim().min(1, "outcome_id is required"),
  ...
  tif: external_exports.string().trim().optional().default("GTC"),
  // Time in force, default to Good Till Cancel
  token: external_exports.string().trim().optional()
});
var onRequestPost4 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    if (market.trading_paused === 1) {
      return errorResponse("Trading is paused for this market", 403);
    }
    const outcomeMarketIds = marketId === "market-total-birdies" ? ["market-total-birdies", "market_total_birdies"] : [marketId];
    ...
    const existingCross = await dbFirst(
      db,
...
function computedSettledProfitCents(netPosition, priceBasis, settledPrice) {
...
__name(computedSettledProfitCents, "computedSettledProfitCents");
__name2(computedSettledProfitCents, "computedSettledProfitCents");
var onRequestGet = /* @__PURE__ */ __name2(async (context) => {
  ...
  let positionsWithPrice = positionsDb.map((p) => {
    ...
    const priceBasis = clampPriceBasis2(p.price_basis, p.net_position);
    const settledProfit = outcomeSettled && p.outcome_settled_price != null ? computedSettledProfitCents(p.net_position, priceBasis, p.outcome_settled_price) : p.settled_profit;
    return {
      id: p.id,
      user_id: p.user_id,
      outcome: p.outcome,
...
var onRequestGet3 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const db = getDb(env);
  try {
    let getOrCreate2 = /* @__PURE__ */ __name(function(userId, marketType) {
      ...
      return s;
    }, "getOrCreate2"), unrealizedPnlRaw2 = /* @__PURE__ */ __name(function(p, currentPrice) {
    ...
    var getOrCreate = getOrCreate2, unrealizedPnlRaw = unrealizedPnlRaw2;
    __name2(getOrCreate2, "getOrCreate");
    __name2(unrealizedPnlRaw2, "unrealizedPnlRaw");
    const users = await dbQuery(
      db,
      "SELECT id, username FROM users ORDER BY username ASC",
      []
    );
    const outcomeMarketTypeRows = await dbQuery(
      db,
    ...
    const sharesByUser = /* @__PURE__ */ new Map();
    sharesRows.forEach((r) => sharesByUser.set(r.user_id, r.total ?? 0));
    const statsByUserMarketType = /* @__PURE__ */ new Map();
    const tradesWithOutcome = await dbQuery(
      db,
    ...
    for (const t of tradesWithOutcome) {
      ...
      if (t.taker_user_id != null) {
        const s = getOrCreate2(t.taker_user_id, mt);
        s.trade_count += 1;
        s.shares_traded += Number(t.contracts) || 0;
      }
      if (t.maker_user_id != null && t.maker_user_id !== t.taker_user_id) {
        const s = getOrCreate2(t.maker_user_id, mt);
        s.trade_count += 1;
        s.shares_traded += Number(t.contracts) || 0;
      }
    }
    const ordersWithOutcome = await dbQuery(
      db,
    ...
    for (const o of ordersWithOutcome) {
      ...
      const s = getOrCreate2(o.user_id, mt);
      s.open_orders_count += 1;
    }
    const positionsRows = await dbQuery(
      db,
    ...
    const portfolioByUser = /* @__PURE__ */ new Map();
    const closedProfitByUser = /* @__PURE__ */ new Map();
    const settledProfitByUser = /* @__PURE__ */ new Map();
    const pnlByOutcome = {};
    const closedProfitByOutcome = {};
    const settledProfitByOutcome = {};
    const positionContributions = [];
    const closedProfitContributions = [];
    const settledProfitContributions = [];
    ...
    for (const p of positionsRows) {
      ...
      const unrealizedRaw = isSettled ? 0 : unrealizedPnlRaw2(p, currentPrice);
      ...
      if (p.user_id == null || !userIds.has(p.user_id)) {
        ...
        const s = getOrCreate2(p.user_id, mt);
    ...
    const systemTotalReported = Math.abs(systemTotalCentsRaw) <= SYSTEM_TOTAL_TOLERANCE_CENTS ? 0 : Math.round(systemTotalCentsRaw);
    const statsByMarketTypePayload = {};
    statsByUserMarketType.forEach((byType, userId) => {
      const obj = {};
      byType.forEach((s, marketType) => {
        obj[marketType] = { ...s };
      });
...
var OFFSET_OUTCOME = "__closed_profit_offset";
...
var onRequestPost8 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const volumeSince = Math.floor(Date.now() / 1e3) - VOLUME_DAYS * 24 * 60 * 60;
  try {
    const volumeRows = await dbQuery(
      db,
    ...
    const markets = await dbQuery(db, "SELECT market_id FROM markets", []);
    const now = Math.floor(Date.now() / 1e3);
...
var onRequestPost9 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const skippedRows = outcomes.length > 0 ? await dbQuery(
      db,
    ...
    const trades = afterTradeId != null ? await dbQuery(
      db,
      `SELECT id, outcome, price, contracts, taker_user_id, maker_user_id, taker_side
    ...
    for (const t of tradesToProcess) {
      ...
      const { riskOffContractsTaker, riskOffContractsMaker, riskOffPriceDiffCents, riskOffPriceDiffMakerCents } = computeRiskOffForFill(
        takerNet,
        takerBasis,
        makerNet,
        makerBasis,
      ...
      tradeUpdates.push({ id: t.id, riskOffContractsTaker, riskOffContractsMaker, riskOffPriceDiffCents, riskOffPriceDiffMakerCents });
    ...
    if (tradeUpdates.length > 0) {
      try {
        await dbBatch(
          db,
          tradeUpdates.map((u) => ({
            sql: "UPDATE trades SET risk_off_contracts_taker = ?, risk_off_contracts_maker = ?, risk_off_price_diff_taker = ?, risk_off_price_diff_maker = ? WHERE id = ?",
            params: [u.riskOffContractsTaker, u.riskOffContractsMaker, u.riskOffPriceDiffCents, u.riskOffPriceDiffMakerCents, u.id]
          }))
        );
      } catch {
        await dbBatch(
          db,
          tradeUpdates.map((u) => ({
            sql: "UPDATE trades SET risk_off_contracts = ?, risk_off_contracts_taker = ?, risk_off_contracts_maker = ?, risk_off_price_diff = ?, risk_off_price_diff_maker = ? WHERE id = ?",
            params: [u.riskOffContractsTaker + u.riskOffContractsMaker, u.riskOffContractsTaker, u.riskOffContractsMaker, u.riskOffPriceDiffCents, u.riskOffPriceDiffMakerCents, u.id]
          }))
        );
      }
    }
    const message = hasMore ? `Replayed ${totalTradesReplayed} trades (chunk). Call again with after_trade_id: ${lastTradeIdProcessed} to continue.` : `Replayed ${totalTradesReplayed} trades. Positions recomputed; risk_off backfilled.${tradesSkipped > 0 ? ` ${tradesSkipped} trade(s) skipped (missing taker_user_id or taker_side).` : ""}`;
    return jsonResponse({
      applied: true,
      message,
      ...
      outcomes_with_skipped: outcomesWithSkipped.length > 0 ? outcomesWithSkipped : void 0,
...
var onRequestGet5 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const token = getCookieValue(cookieHeader, "session");
  if (!token) {
    const response2 = jsonResponse({ user: null });
...
var onRequestGet6 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const db = getDb(env);
  const tables = ["orders", "trades", "positions", "outcomes", "markets", "users"];
  const schema = {};
...
function slugForOutcomeId2(s) {
  return (s || "").trim().toLowerCase().replace(/\s+/g, "-").replace(/\./g, "_").replace(/[^a-z0-9_-]/g, "").replace(/-+/g, "-").replace(/^\-|\-$/g, "") || "x";
}
__name(slugForOutcomeId2, "slugForOutcomeId2");
__name2(slugForOutcomeId2, "slugForOutcomeId");
function participantFromTotalBirdiesName(name) {
  const overIdx = (name || "").indexOf(" Over ");
  if (overIdx === -1) return (name || "").trim();
  return (name || "").slice(0, overIdx).trim();
}
__name(participantFromTotalBirdiesName, "participantFromTotalBirdiesName");
__name2(participantFromTotalBirdiesName, "participantFromTotalBirdiesName");
var onRequestPost13 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const outcomeIds = [];
    const createdDate = Math.floor(Date.now() / 1e3);
    for (const outcome of validated.outcomes) {
      let outcomeId;
      if (isTotalBirdies) {
        const participant = participantFromTotalBirdiesName(outcome.name);
        const participantSlug = slugForOutcomeId2(participant);
        const strikeSlug = outcome.strike ? slugForOutcomeId2(outcome.strike) : "";
        outcomeId = strikeSlug ? `outcome-total-birdies-${participantSlug}-${strikeSlug}` : `outcome-total-birdies-${participantSlug}`;
      } else {
...
var onRequestGet7 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    if (yearParam === "all") {
      ...
      const handicapsByYear = {};
      for (const r of rows2) {
        const y = String(r.year);
        if (!handicapsByYear[y]) handicapsByYear[y] = {};
        handicapsByYear[y][r.player] = r.handicap_index;
      }
      const response2 = jsonResponse({ handicapsByYear });
      response2.headers.set("Cache-Control", `public, max-age=${CACHE_MAX_AGE}, stale-while-revalidate=86400`);
      return response2;
    }
...
var onRequestGet10 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const volatilityByPlayer = {};
    for (const r of rows) {
      volatilityByPlayer[r.player] = { year: r.year, volatility: r.volatility };
    }
    const response = jsonResponse({ volatilityByPlayer });
    response.headers.set("Cache-Control", `public, max-age=${CACHE_MAX_AGE2}, stale-while-revalidate=86400`);
    return response;
  } catch (err) {
...
function computedSettledProfitCents2(netPosition, priceBasis, settledPrice) {
  if (netPosition > 0 && priceBasis > 0) {
    return netPosition * (settledPrice - priceBasis);
  }
  if (netPosition < 0 && priceBasis > 0) {
...
__name(computedSettledProfitCents2, "computedSettledProfitCents2");
__name2(computedSettledProfitCents2, "computedSettledProfitCents");
var onRequestGet11 = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    const volRow = await dbFirst(db, "SELECT volume_contracts FROM market_volume WHERE market_id = ?", [marketId]);
    if (volRow?.volume_contracts != null) volume_contracts = volRow.volume_contracts;
  } catch {
  }
  market.volume_contracts = volume_contracts;
  const outcomeMarketIds = marketId === "market-total-birdies" ? ["market-total-birdies", "market_total_birdies"] : [marketId];
  ...
  const lastTradePriceByOutcome = {};
  const tradesByOutcomeDesc = [...tradesDb].sort((a, b) => (b.create_time ?? 0) - (a.create_time ?? 0));
  for (const t of tradesByOutcomeDesc) {
    const oid = t.outcome ?? "";
    if (oid && lastTradePriceByOutcome[oid] === void 0) lastTradePriceByOutcome[oid] = t.price;
  }
  let positions = [];
  if (currentUserId != null) {
    ...
    if (positionsDb.length > 0) {
      ...
      positions = positionsDb.map((p) => {
        const outcomeSettled = p.outcome_settled_price != null;
        const price_basis = p.net_position !== 0 && p.price_basis > 0 ? Math.max(100, Math.min(9900, p.price_basis)) : p.price_basis;
        const settledProfit = outcomeSettled && p.outcome_settled_price != null ? computedSettledProfitCents2(p.net_position, price_basis, p.outcome_settled_price) : p.settled_profit;
        const bidPrice = bestBidByOutcome[p.outcome] ?? null;
        const askPrice = bestAskByOutcome[p.outcome] ?? null;
        const midPrice = bidPrice != null && askPrice != null ? (bidPrice + askPrice) / 2 : bidPrice ?? askPrice ?? null;
        const current_price = outcomeSettled ? p.outcome_settled_price : midPrice;
...
var onRequestGet13 = /* @__PURE__ */ __name2(async (context) => {
  try {
    ...
    const outcomeQueryIds = marketIds.includes("market-total-birdies") && !marketIds.includes("market_total_birdies") ? [...marketIds, "market_total_birdies"] : marketIds;
    let allOutcomes = [];
...
function computedSettledProfitCents3(netPosition, priceBasis, settledPrice) {
  if (netPosition > 0 && priceBasis > 0) {
    return netPosition * (settledPrice - priceBasis);
  }
  if (netPosition < 0 && priceBasis > 0) {
...
__name(computedSettledProfitCents3, "computedSettledProfitCents3");
__name2(computedSettledProfitCents3, "computedSettledProfitCents");
var onRequestGet16 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const normalizeMarketId = /* @__PURE__ */ __name2((p) => p.market_id === "market_total_birdies" ? "market-total-birdies" : p.market_id, "normalizeMarketId");
  ...
  let positionsWithOrderbook = positionsDb.map((p) => {
    const outcomeSettled = p.outcome_settled_price != null;
    const priceBasis = clampBasis(p);
    const settledProfit = outcomeSettled && p.outcome_settled_price != null ? computedSettledProfitCents3(p.net_position, priceBasis, p.outcome_settled_price) : p.settled_profit;
...
var onRequestGet18 = /* @__PURE__ */ __name2(async (context) => {
  ...
  const trades = tradesRows.map((t) => {
    const mySide = t.taker_side != null ? t.taker_user_id === userId ? t.taker_side : t.taker_side === 0 ? 1 : 0 : null;
    const market_id = t.market_id === "market_total_birdies" ? "market-total-birdies" : t.market_id;
...
// .wrangler/tmp/bundle-8uGZYE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-8uGZYE/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2986485560156067.js.map
