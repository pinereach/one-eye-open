...
// ../.wrangler/tmp/bundle-d6rvTo/checked-fetch.js
  ...
  if (positionsDb.length > 0) {
    ...
    positionsWithPrice = positionsDb.map((p) => {
      ...
      return {
        ...
        settled_profit: p.settled_profit,
        net_position: p.net_position,
        price_basis: clampPriceBasis2(p.price_basis, p.net_position),
        is_settled: p.is_settled,
        market_name: p.market_name,
        outcome_name: p.outcome_name,
        outcome_ticker: p.outcome_ticker,
...
// api/markets/[marketId]/reference.ts
var CACHE_MAX_AGE = 43200;
...
var loginSchema = external_exports.object({
  username: external_exports.string().trim().min(1, "Username is required"),
  password: external_exports.string().trim().min(1, "Password is required")
});
...
var registerSchema = external_exports.object({
  username: external_exports.string().trim().min(1, "Username is required").max(100, "Username too long"),
  password: external_exports.string().trim().min(1, "Password is required")
});
...
var outcomeSchema = external_exports.object({
  name: external_exports.string().trim().min(1, "Outcome name is required"),
  ticker: external_exports.string().trim().min(1, "Ticker is required"),
  strike: external_exports.string().trim().optional().default(""),
  outcome_id: external_exports.string().trim().min(1).optional()
...
var marketSuggestionSchema = external_exports.object({
  short_name: external_exports.string().trim().min(1, "Market name is required"),
  symbol: external_exports.string().trim().min(1, "Symbol is required"),
...
var onRequestPost9 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const isH2H = validated.outcomes.some((o) => o.outcome_id?.startsWith("outcome-h2h-"));
...
var onRequestGet7 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    if (yearParam === "all") {
      ...
      response2.headers.set("Cache-Control", `public, max-age=${CACHE_MAX_AGE2}`);
      return response2;
    }
    ...
    response.headers.set("Cache-Control", `public, max-age=${CACHE_MAX_AGE2}`);
    return response;
  } catch (err) {
...
// ../.wrangler/tmp/pages-WWvSNA/functionsRoutes-0.8869817360511045.mjs
var routes = [
  {
  ...
  {
    routePath: "/api/markets/:marketId/reference",
    mountPath: "/api/markets/:marketId",
    method: "GET",
    middlewares: [],
...
// ../.wrangler/tmp/bundle-d6rvTo/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-d6rvTo/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.873742328007312.mjs.map
