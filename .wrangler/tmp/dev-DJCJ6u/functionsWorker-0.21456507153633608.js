...
// .wrangler/tmp/bundle-zsFvNn/checked-fetch.js
...
// .wrangler/tmp/pages-tlo0zd/functionsWorker-0.21456507153633608.mjs
...
function isAuthRequired(env, request) {
  const isProduction = env.ENVIRONMENT === "production" || !request.url.includes("localhost") && !request.url.includes("127.0.0.1") && !request.url.includes(":8788") && !request.url.includes(":3000");
  return isProduction;
}
__name(isAuthRequired, "isAuthRequired");
__name2(isAuthRequired, "isAuthRequired");
...
async function createTrade(db, _marketId, _takerOrderId, _makerOrderId, priceCents, qtyContracts) {
  const token = crypto.randomUUID();
  const result = await dbRun(
    db,
    `INSERT INTO trades (token, price, contracts, create_time, risk_off_contracts, risk_off_price_diff)
     VALUES (?, ?, ?, ?, 0, 0)`,
    [token, priceCents, qtyContracts, Math.floor(Date.now() / 1e3)]
  );
  ...
  for (const fill of matchedFills) {
    ...
    if (makerOrderDb) {
      ...
      await updatePosition(
        db,
        outcomeId,
        // Use outcomeId passed to executeMatching
        takerUserId,
        takerOrder.side,
        fill.price_cents,
        fill.qty_contracts
...
async function calculateMarketPnL(db, marketId, settleValue) {
  const positions = await dbQuery(
    db,
...
__name(calculateMarketPnL, "calculateMarketPnL");
__name2(calculateMarketPnL, "calculateMarketPnL");
...
// .wrangler/tmp/bundle-zsFvNn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-zsFvNn/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.21456507153633608.js.map
